 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Symfony2_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
        ...
        while (($nextParam = $phpcsFile->findNext(T_VARIABLE, ($nextParam + 1), $closeBracket)) !== false) {
            ...
            if ($nextComma !== false) {
                ...
                if ($tokens[($nextComma + 1)]['code'] !== T_WHITESPACE) {
                    $error = 'Expected 1 space between argument "%s" and comma; %s found';
                    $data  = array(
                              $tokens[$nextParam]['content'],
                ...
                if ($whitespace['code'] === T_WHITESPACE) {
                    ...
                    if ($nextToken !== $nextParam) {
                        ...
                        if ($multiLine === false
                            ...
                            $error = 'Expected 0 space between opening bracket and type hint "%s"; %s found';
                            $data  = array(
                                      $hint,
                        ...
                        $error = 'Expected 0 space between opening bracket and argument "%s"; %s found';
                        $data  = array(
                                      $arg,
                                      $gap,
                                     );
